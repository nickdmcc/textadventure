...
public class Assassin extends PlayerClass{
	...
	public Assassin(String name, String weapon, String className, int health, int maxHealth, int damage, int critical, int run)
	{
		setName(name);
		setWeapon(weapon);
	...
	public void skill1(Monster monster)
	{
		...
		if (getEnergy() >= 4)
		{
			attack = (criticalRole(attackDamageRange()));
			monsterAttack = monster.attackDamageRange();
			
			if (isCritical())
			{
				setAttackMessage((getName() + " uses Perfect Strike! You land a critical dealing " + attack + " damage\nCritical damage!\n"));
			}
			else
			{	
				attack = attack * 2;
